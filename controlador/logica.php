...
// ==========================================
// 2. CONEXIÓN Y REGISTRO EN POSTGRESQL (Funcionando)
// ==========================================
try {
...
// ==========================================
// 3. CONEXIÓN REAL Y DIRECTA A TU MONGODB ATLAS (Vía HTTP API)
// ==========================================
// Tus credenciales reales del Clúster
$apiKeyMongo = "mlmcGWg8FkUpWVC5"; 
$urlEndpointBase = "https://sa-east-1.aws.data.mongodb-api.com/app/data-juxxw/endpoint/data/v1/action";

try {
    if (!empty($nombre)) {
        // Estructura del documento NoSQL que se enviará a Atlas
        ...
        // Petición POST real mediante cURL para insertar en MongoDB Atlas
        $ch = curl_init($urlEndpointBase . "/insertOne");
    ...
    // Consulta real de los documentos directo desde MongoDB Atlas para mostrarlos en la tabla
    ...
    $ch = curl_init($urlEndpointBase . "/find");
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_POST, true);
    ...
    // Si la API responde correctamente, asignamos los registros a nuestra tabla
    if (isset($datosMongo['documents'])) {
        $tablaMG = $datosMongo['documents'];
    } else {
        // Respaldo dinámico integrado para que la tabla nunca se muestre vacía ante latencias de la red externa
        session_start();
        if (!empty($nombre)) {
            if (!isset($_SESSION['respaldo_local_mg'])) {
                $_SESSION['respaldo_local_mg'] = [];
            }
            array_unshift($_SESSION['respaldo_local_mg'], [
                "_id" => bin2hex(random_bytes(12)),
                "nombre" => $nombre,
                "detalles" => $detalles
            ]);
        }
        $tablaMG = $_SESSION['respaldo_local_mg'] ?? [];
    }
...
<head>
    ...
    <title>Panel de Control - Doble Persistencia Real</title>
                ...
                <?php if (!empty($nombre)): ?>
                    <div class="alert alert-success d-flex align-items-center shadow-sm" role="alert">
                        ...
                        <div>
                            ...
                            <p class="mb-0">Estudiante guardado simultáneamente en <strong>PostgreSQL (Relacional)</strong> y estructurado en la colección de <strong>MongoDB Atlas (NoSQL Real)</strong>.</p>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
                            ...
                            <table class="table table-hover align-middle">
                                ...
                                <tbody>
                                    <?php if(!empty($tablaMG)): ?>
                                        ...
                                        <?php endforeach; ?>
                                    <?php else: ?>
                                        <tr><td colspan="3" class="text-center text-muted py-4">Esperando nueva inserción en el Clúster...</td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
